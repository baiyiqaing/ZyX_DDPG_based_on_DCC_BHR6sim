...
float fComStatedelta[4];
float fZmpStatedelta[2];
double dOri[2];
int nSuplegZyx;
float fPstState[2][4]; // [ref, sens][pit, rol, dpit, drol] 
float fFootFTSens[2][6]; // [r, l][fx, fy, fz, tx, ty, tz]
float fFootFTRef[2][6]; // [r, l][fx, fy, fz, tx, ty, tz]

float fComStateSens[6];
float fComStateRef[6];

float fZmpStateSens[2];
float fZmpStateRef[2];
...
void fnvLipmCon(double dParasLipm_in[6], double dLimitsLipm_in[4]) {
	...
	fPstState[0][0] = (float)stStateRef.Base.rot.pit, fPstState[0][1] = (float)stStateRef.Base.rot.rol, fPstState[0][2] = (float)stStateRef.Base.rot.dpit, fPstState[0][3] = (float)stStateRef.Base.rot.drol;
	fPstState[1][0] = (float)stStateSens.Base.rot.pit, fPstState[1][1] = (float)stStateSens.Base.rot.rol, fPstState[1][2] = (float)stStateSens.Base.rot.dpit, fPstState[1][3] = (float)stStateSens.Base.rot.drol;
	...
	fComStateRef[0] = (float)stStatePG.Base.pos.x;
	fComStateRef[1] = (float)stStatePG.Base.pos.y;
	fComStateRef[2] = (float)stStatePG.Base.pos.z;
	fComStateRef[3] = (float)stStatePG.Base.pos.dx;
	fComStateRef[4] = (float)stStatePG.Base.pos.dy;
	fComStateRef[5] = (float)stStatePG.Base.pos.dz;
	// 20220108 zyx modify

	fComStatedelta[0] = (float)delta_cx;
	fComStatedelta[1] = (float)delta_cy;
	//fComStatedelta[2] = (float)delta_cz;
	fComStatedelta[2] = (float)delta_vx;
	fComStatedelta[3] = (float)delta_vy;
	//fComStatedelta[5] = (float)delta_vz;

	
	fComStateSens[0] = (float)stStatePG.Base.pos.x + (float)delta_cx;
	fComStateSens[1] = (float)stStatePG.Base.pos.y + (float)delta_cy;
	fComStateSens[2] = (float)stStatePG.Base.pos.z;
	fComStateSens[3] = (float)stStatePG.Base.pos.dx + (float)delta_vx;
	fComStateSens[4] = (float)stStatePG.Base.pos.dy + (float)delta_vy;
	fComStateSens[5] = (float)stStatePG.Base.pos.dz;

	  // Zmp

	fZmpStatedelta[0] = (float)stStateSens.ZMP.B.x - (float)stStatePG.ZMP.B.x;
	fZmpStatedelta[1] = (float)stStateSens.ZMP.B.y - (float)stStatePG.ZMP.B.y;

	fZmpStateSens[0] = (float)stStateSens.ZMP.B.x;
	fZmpStateSens[1] = (float)stStateSens.ZMP.B.y;

	fZmpStateRef[0] = (float)stStatePG.ZMP.B.x;
	fZmpStateRef[1] = (float)stStatePG.ZMP.B.y;

      // - -
	dOri[0] = stStateSens.Base.rot.pit;
	dOri[1] = stStateSens.Base.rot.rol;
}
...
void fnvGrfCon(double dParasGrfC[7], double dParasVarStiff[2], double dLimitsGrfC[7], double dLimitAddiTrq[4], double dThreshGrfC[6], double dVarStiffLat_T) {
	...
	// zyx
	fFootFTSens[0][0] = (float)stStateSens.FootFT.Rfoot.fx, fFootFTSens[0][1] = (float)stStateSens.FootFT.Rfoot.fy, fFootFTSens[0][2] = (float)stStateSens.FootFT.Rfoot.fz;
	fFootFTSens[0][3] = (float)stStateSens.FootFT.Rfoot.tx, fFootFTSens[0][4] = (float)stStateSens.FootFT.Rfoot.ty, fFootFTSens[0][5] = (float)stStateSens.FootFT.Rfoot.tz;
	fFootFTRef[0][0]  = (float)stStateRef.FootFT.Rfoot.fx,  fFootFTRef[0][1]  = (float)stStateRef.FootFT.Rfoot.fy,  fFootFTRef[0][2]  = (float)stStateRef.FootFT.Rfoot.fz;
	fFootFTRef[0][3]  = (float)stStateRef.FootFT.Rfoot.tx,  fFootFTRef[0][4]  = (float)stStateRef.FootFT.Rfoot.ty,  fFootFTRef[0][5]  = (float)stStateRef.FootFT.Rfoot.tz;
	fFootFTSens[1][0] = (float)stStateSens.FootFT.Lfoot.fx, fFootFTSens[1][1] = (float)stStateSens.FootFT.Lfoot.fy, fFootFTSens[1][2] = (float)stStateSens.FootFT.Lfoot.fz;
	fFootFTSens[1][3] = (float)stStateSens.FootFT.Lfoot.tx, fFootFTSens[1][4] = (float)stStateSens.FootFT.Lfoot.ty, fFootFTSens[1][5] = (float)stStateSens.FootFT.Lfoot.tz;
	fFootFTRef[1][0]  = (float)stStateRef.FootFT.Lfoot.fx,  fFootFTRef[1][1]  = (float)stStateRef.FootFT.Lfoot.fy,  fFootFTRef[1][2]  = (float)stStateRef.FootFT.Lfoot.fz;
	fFootFTRef[1][3]  = (float)stStateRef.FootFT.Lfoot.tx,  fFootFTRef[1][4]  = (float)stStateRef.FootFT.Lfoot.ty,  fFootFTRef[1][5]  = (float)stStateRef.FootFT.Lfoot.tz;
	// update
...
void fnvDccRunCon() {
	...
	// paras Limp
	double dParasLipm[6] = { /*kp_x, kv_x, kz_x*/2.5 * 1.1, 0.8 * 1.1, -0.022917, /*kp_y, kv_y, kz_y*/2.5 * 1.1, 0.8 * 1.1, -0.022917 }; // CAN SHU
	// Zyx rec 一旦跳出deng dai的循环，就会刷新参数，顺序执行
	for (int dcci = 0; dcci < 3; dcci++) dParasLipm[dcci] = dParasLipm[dcci + 3] = dZyxDataIn[dcci + 1];
