...
static void uiUpdateState(GLFWwindow* wnd)
{
    ...
    // get mouse position, scale by buffer-to-MJwindow ratio
    double x, y;
...
static void uiScroll(GLFWwindow* wnd, double xoffset, double yoffset)
{
    ...
    // set scroll info, scale by buffer-to-MJwindow ratio
...
{
    // compute framebuffer-to-MJwindow ratio
...
// Set internal and user-supplied UI callbacks in GLFW MJwindow.
void uiSetCallback(GLFWwindow* wnd, mjuiState* state, 
                   uiEventFn uiEvent, uiLayoutFn uiLayout)
{
    // make container with user-supplied objects and set MJwindow pointer
    ...
    // compute framebuffer-to-MJwindow pixel ratio
    int width_win, width_buf, height;
    glfwGetWindowSize(wnd, &width_win, &height);
    glfwGetFramebufferSize(wnd, &width_buf, &height);
...
// Clear UI callbacks in GLFW MJwindow.
