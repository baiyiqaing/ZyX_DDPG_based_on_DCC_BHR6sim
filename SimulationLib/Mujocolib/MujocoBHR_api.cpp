...
void fnvMujocoJointsCon(double dKp, double dKd)
{
            ...
            if( settings.run )
            {
                ...
                else
                {
                           ...
                           (glfwGetTime()-tmstart)<refreshfactor/vmode.refreshRate )
                    {
                        ...
                        if(k_pre == 0) for(int i = 1; i < m->nq + 7; i++) d->qpos[i] = _dJointsInitPos[i];
                        for(int i = 7; i < m->nq + 7; i++) dptJointsPosition[i - 7] = d->qpos[i], dptJointsVelocity[i - 7] = d->qvel[i]; // read joints
                        for(int i = 0; i < 3; i++) for(int j = 0; j < 3; j++) dRotMat[i][j] = d->site_xmat[3 * i + j]; // read trunk rot mat
                        dptGyroSensor[0][1] = atan2(dRotMat[2][1], dRotMat[2][2]); // process IMU pitch
                        dptGyroSensor[0][0] = atan2(-dRotMat[2][0], sqrt(dRotMat[2][1] * dRotMat[2][1] + dRotMat[2][2] * dRotMat[2][2])); // process IMU roll
                        dptGyroSensor[0][2] = atan2(dRotMat[1][0], dRotMat[0][0]); // process IMU yaw
                        for(int i = 0; i < 6; i++) {
